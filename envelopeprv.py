...
def compute_pair(args):
    i, j, r, corr_thr, ampl_thr = args
    maxsl0 = np.max(r[i].data)
    maxsl1 = np.max(r[j].data)

    if maxsl1 >= ampl_thr and maxsl0 >= ampl_thr:
        cc, _ = xcorr(signal.detrend(r[i].data), signal.detrend(r[j].data))
        corr = np.max(cc)
        if corr > corr_thr:
            return {
                'id': r[i].id + '_' + r[j].id,
...
def trARatio_mp(r, corr_thr, ampl_thr):
    n = len(r)
    pairs = [(i, j, r, corr_thr, ampl_thr) for i in range(n) for j in range(i + 1, n)]

    with Pool(5) as pool:
        results = pool.map(compute_pair, pairs)

    c = [res for res in results if res is not None]

    # Ricostruisci dizionario s
    s = {}
    for item in c:
        i, j = item['id'].split('_')
        s[i] = item['max0']
...
print("elapsed "+str(UTCDateTime.now()-t0))
